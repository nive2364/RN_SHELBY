// Define a namespace for the report to be part of the Inventory module
namespace Microsoft.Inventory.Reports;

// Import necessary modules for finance, ledger, item, location, and utility functions
...
// Define the report with a unique ID, name, and attributes
report 50123 "On Hand Inventory"
{
    DefaultLayout = RDLC; // Specifies the report layout as RDLC (client report definition)
    RDLCLayout = './Local/Inventory/Reports/OnHandInventory.rdlc'; // Path to the RDLC layout file
    ApplicationArea = All; // Makes the report accessible across all application areas
    Caption = 'On Hand Inventory'; // The title of the report
    ...
    dataset // Define the dataset for the report, consisting of data items and columns
    {
        dataitem(Item; Item) // Define a data item for the Item table
        {
            ...
            RequestFilterFields = "No.", "Inventory Posting Group", "Costing Method", "Location Filter", "Variant Filter", "Lot No. Filter"; // Fields available for filtering
            ...
            column(ShowLocations; ShowLocations2)
            {
            }
            ...
            dataitem("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                begin
                    ...
                    GetOnHandInventory.UpdateBuffer("Item Ledger Entry", TempEntryBuffer, ShowVariants, ShowLocations2, ShowLotNos, ShowPackageNos, LastItemNo, LastLocationCode, LastVariantCode, LastLotNo, LastPackageNo, VariantLabel, LocationLabel, IsCollecting, NoVariantLbl, NoLocationLbl); // Update buffer with the current item ledger entry
            ...
            dataitem(BufferLoop; "Integer")
            {
                ...
                begin
                    ...
                    if Item."Location Filter" <> '' then
                        TempEntryBuffer.SetFilter("Location Code", '%1', Item."Location Filter"); // Apply location filter if set

                    if Item."Variant Filter" <> '' then
                        TempEntryBuffer.SetFilter("Variant Code", '%1', Item."Variant Filter"); // Apply variant filter if set
        ...
        layout // Define the layout for the request page
        {
            area(content) // Define content area of the request page
            {
                group(Options) // Group the options section
                {
                    ...
                    field(BreakdownByVariants; ShowVariants) // Field for selecting breakdown by item variants
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Breakdown by Variants'; // Field caption
                        ToolTip = 'Specifies the item variants that you want the report to consider.'; // Tooltip description
                        Visible = false; // Hide this option by default
                    }
                    field(BreakdownByLocation; ShowLocations2) // Field for selecting breakdown by locations
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Breakdown by Location'; // Field caption
                        ToolTip = 'Specifies the breakdown report data by locations.'; // Tooltip description
                        Visible = true; // Make this field visible
                    }
                    field(BreakdownByLot; ShowLotNos) // Field for selecting breakdown by locations
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Breakdown by Lot Number'; // Field caption
                        ToolTip = 'Specifies the breakdown report data by lot number.'; // Tooltip description
                        Visible = true; // Make this field visible
                        trigger OnValidate()
                        begin
                            EnablePackageNos := ShowLotNos;
                        end;
                    }
                    field(BreakdownByPackage; ShowPackageNos) // Field for selecting breakdown by locations
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Breakdown by Package Number'; // Field caption
                        ToolTip = 'Specifies the breakdown report data by package number.'; // Tooltip description
                        Enabled = EnablePackageNos; // Enable this field based on the lot number selection
                    }
                    field(UseAdditionalReportingCurrency; ShowACY) // Option to use an additional reporting currency
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Use Additional Reporting Currency'; // Field caption
                        ToolTip = 'Specifies if you want all amounts to be printed by using the additional reporting currency. If you do not select the check box, then all amounts will be printed in US dollars.'; // Explanation for this option
    ...
    begin
        ...
        if ShowLocations and (not ShowVariants) then
            if not "Item Ledger Entry".SetCurrentKey("Item No.", "Location Code") then
                Error(ShowLocationAndVariantsErr, "Item Ledger Entry".TableCaption(), "Item Ledger Entry".FieldCaption("Item No."), "Item Ledger Entry".FieldCaption("Location Code")); // Error if a valid key for location and item is missing

        if Item.GetFilter("Date Filter") <> '' then
            Error(EndDateFilterErr, Item.FieldCaption("Date Filter"), Item.TableCaption()); // Error if a Date Filter is set instead of using As Of Date
    ...
    var // Define global variables used in the report
        ...
        ShowLocations2: Boolean; // Boolean flag for showing locations - parsed from Package Number
