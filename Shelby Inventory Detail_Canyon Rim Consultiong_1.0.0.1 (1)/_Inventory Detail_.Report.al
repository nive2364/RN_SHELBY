report 50121 "Inventory Detail"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Local/Inventory/Reports/ShelbyInventoryDetail.rdlc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            RequestFilterFields = "No.", "Date Filter", "Inventory Posting Group";

            // Columns for the Item dataitem
            ...
            dataitem(PriorItemLedgerEntry; "Item Ledger Entry")
            {
                ...
                column(Prior_Lot_No_; "Lot No.")
                {
                }
                column(Prior_Expiration_Date; "Expiration Date")
                {
                }
                ...
                begin
                    ...
                    if ValueEntry.FindSet()then begin
                        repeat Adjustment:=Adjustment + ValueEntry."Cost Amount (Actual)";
                        until ValueEntry.Next() = 0;
                        if Adjustment <> 0 then ValueOnHand:=ValueOnHand + Adjustment
                        else
                            CurrReport.Skip();
                    end
                    else
                ...
                begin
                    if ItemLedgerFilter <> '' then CurrReport.Break();
                    if not SetCurrentKey("Item No.", "Variant Code", "Global Dimension 1 Code", "Global Dimension 2 Code", "Location Code", "Posting Date")then SetCurrentKey("Item No.", "Variant Code", "Location Code", "Posting Date");
            ...
            dataitem("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                DataItemTableView = sorting("Item No.", "Variant Code", "Location Code", "Posting Date");
                RequestFilterFields = "Entry Type", "Lot No.", "Package No.", "Expiration Date";
                ...
                begin
                    ...
                    if "Invoiced Quantity" <> Quantity then "Cost Amount (Actual)":="Cost Amount (Expected)";
                    ...
                    if PreviousVariantCode <> "Variant Code" then begin
                        ...
                        if ItemVariant.Get("Item No.", "Variant Code") and ("Variant Code" <> '')then ItemVariantFlag:=true;
            ...
            begin
                if ItemLedgerFilter = '' then begin
                    ...
                    if not ValueEntry.SetCurrentKey("Item No.", "Posting Date", "Item Ledger Entry Type", "Entry Type", "Variance Type", "Item Charge No.", "Location Code", "Variant Code", "Global Dimension 1 Code", "Global Dimension 2 Code")then ValueEntry.SetCurrentKey("Item No.", "Posting Date", "Item Ledger Entry Type", "Entry Type", "Variance Type", "Item Charge No.", "Location Code", "Variant Code");
                    ValueEntry.CalcSums("Cost Amount (Actual)");
                    ValueOnHand:=ValueEntry."Cost Amount (Actual)";
                end
                else
                begin // If the user enters any filters for Item Ledger Entries,
                ...
                if PrintOnlyOnePerPage then NewPagePerRecordNo:=NewPagePerRecordNo + 1;
            end;
            trigger OnPreDataItem()
            begin
