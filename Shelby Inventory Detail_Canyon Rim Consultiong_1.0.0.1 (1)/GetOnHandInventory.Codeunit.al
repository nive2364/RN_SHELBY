codeunit 50124 GetOnHandInventory
{
    ...
    begin
        ...
        begin
            ...
            if ItemApplicationEntry.Find('-')then repeat if ItemLedgerEntry2.Get(ItemApplicationEntry."Inbound Item Entry No.") and (ItemLedgerEntry2."Posting Date" <= EndDate)then ItemLedgerEntry."Remaining Quantity":=ItemLedgerEntry."Remaining Quantity" - ItemApplicationEntry.Quantity;
    ...
    begin
        ...
        if ShowVariants or ShowLocations or ShowLotNos then begin
            ...
            if ItemLedgEntry."Variant Code" <> LastVariantCode then begin
                ...
                if ShowVariants then if(ItemLedgEntry."Variant Code" = '') or (not ItemVariant.Get(ItemLedgEntry."Item No.", ItemLedgEntry."Variant Code"))then VariantLabel:=NoVariantLbl
                    else
            ...
            if ItemLedgEntry."Location Code" <> LastLocationCode then begin
                ...
                if ShowLocations then begin
                    if(ItemLedgEntry."Location Code" = '') or not Location.Get(ItemLedgEntry."Location Code")then LocationLabel:=NoLocationLbl
                    else
        ...
        if NewRow then UpdateTempEntryBuffer(TempEntryBuffer, IsCollecting);
        ...
        if ShowACY then TempEntryBuffer.Value1+=ItemLedgEntry."Cost Amount (Actual) (ACY)"
        else
    ...
    begin
        ...
        if IsCollecting and ((TempEntryBuffer."Remaining Quantity" <> 0) or (TempEntryBuffer.Value1 <> 0))then TempEntryBuffer.Insert();
        IsCollecting:=false;
